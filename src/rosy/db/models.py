...
``guild_id`` (a Discord snowflake). All Discord snowflake ids are stored as
BIGINT (64-bit) because 32-bit INTEGER overflows on large ids — this is a
fatal bug if you use plain ``Integer``.

DM data carries ``guild_id=None``.
"""
...
# Discord snowflake ids are 64-bit. Use BIGINT on PostgreSQL (correct for large
# ids) but INTEGER on SQLite (so local tests get auto-incrementing PKs).
...
class Guild(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
...
class Channel(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
    guild_id: Mapped[Optional[int]] = mapped_column(Snowflake, ForeignKey("guilds.id"), nullable=True)
    channel_id: Mapped[int] = mapped_column(Snowflake)
    name: Mapped[str] = mapped_column(String(200), default="")
...
class Conversation(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
    guild_id: Mapped[Optional[int]] = mapped_column(Snowflake, ForeignKey("guilds.id"), nullable=True)
    user_id: Mapped[Optional[int]] = mapped_column(Snowflake, ForeignKey("users.id"), nullable=True)
    channel_id: Mapped[Optional[int]] = mapped_column(Snowflake, nullable=True)
...
class Message(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
    conversation_id: Mapped[int] = mapped_column(Snowflake, ForeignKey("conversations.id"))
...
class Reminder(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
    guild_id: Mapped[Optional[int]] = mapped_column(Snowflake, ForeignKey("guilds.id"), nullable=True)
    user_id: Mapped[int] = mapped_column(Snowflake)
    channel_id: Mapped[int] = mapped_column(Snowflake)
...
class ModerationRecord(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Snowflake, primary_key=True)
    guild_id: Mapped[int] = mapped_column(Snowflake, ForeignKey("guilds.id"))
    user_id: Mapped[int] = mapped_column(Snowflake)
    moderator_id: Mapped[int] = mapped_column(Snowflake)
